...
type Registration = { id: string; student_name: string; whatsapp_number: string; ip_address?: string; created_at: string }
...
export default function AdminDashboard() {
  ...
  const filtered = useMemo(() => { const term = query.trim().toLowerCase(); return term ? registrations.filter(item => item.student_name.toLowerCase().includes(term) || item.whatsapp_number.includes(term) || (item.ip_address && item.ip_address.includes(term))) : registrations }, [registrations, query])
  ...
  const exportCsv = () => { const rows = [['Student Name', 'WhatsApp Number', 'IP Address', 'Registered'], ...filtered.map(item => [item.student_name, item.whatsapp_number, item.ip_address || '', item.created_at])]; const escape = (v: string) => `"${v.replace(/"/g, '""')}"`; const blob = new Blob([rows.map(row => row.map(escape).join(',')).join('\n')], { type: 'text/csv;charset=utf-8' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'grade-6-registrations.csv'; link.click(); URL.revokeObjectURL(link.href) }
  ...
  return <AdminShell><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-wider text-leaf">Private</p><h1 className="mt-1 text-3xl font-bold">Registrations</h1><p className="mt-1 text-slate-600">{registrations.length} total registration{registrations.length === 1 ? '' : 's'}</p></div><button onClick={() => void supabase?.auth.signOut()} className="rounded-lg border px-3 py-2 text-sm font-semibold">Sign out</button></div><div className="mt-7 flex flex-col gap-3 sm:flex-row"><input className="h-12 flex-1 rounded-xl border px-4" placeholder="Search name, WhatsApp or IP address" value={query} onChange={e => setQuery(e.target.value)} /><button onClick={exportCsv} className="h-12 rounded-xl bg-ink px-4 font-bold text-white">Export CSV</button></div><div className="mt-5 overflow-x-auto rounded-xl border"><table className="w-full min-w-[640px] text-left text-sm"><thead className="bg-mist text-slate-700"><tr><th className="px-4 py-3">Student Name</th><th className="px-4 py-3">WhatsApp Number</th><th className="px-4 py-3">IP Address</th><th className="px-4 py-3">Registered</th><th className="px-4 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody>{loading ? <tr><td colSpan={5} className="px-4 py-7 text-center text-slate-500">Loading registrations…</td></tr> : filtered.length ? filtered.map(item => <tr key={item.id} className="border-t"><td className="px-4 py-3 font-medium">{item.student_name}</td><td className="px-4 py-3">{item.whatsapp_number}</td><td className="px-4 py-3 font-mono text-xs text-slate-600">{item.ip_address || '-'}</td><td className="px-4 py-3 text-slate-600">{formatDate(item.created_at)}</td><td className="px-4 py-3 text-right"><button onClick={() => void remove(item.id)} className="font-semibold text-red-700">Delete</button></td></tr>) : <tr><td colSpan={5} className="px-4 py-7 text-center text-slate-500">No matching registrations.</td></tr>}</tbody></table></div></AdminShell>
}
