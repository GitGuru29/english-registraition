...
export default function AdminDashboard() {
  ...
  const exportCsv = () => { const rows = [['Student Name', 'WhatsApp Number', 'Registered'], ...filtered.map(item => [item.student_name, item.whatsapp_number, item.created_at])]; const escape = (v: string) => `"${v.replace(/"/g, '""')}"`; const blob = new Blob([rows.map(row => row.map(escape).join(',')).join('\n')], { type: 'text/csv;charset=utf-8' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'grade-6-registrations.csv'; link.click(); URL.revokeObjectURL(link.href) }
